                        ...
                        <div id="two-column-menu">
                            <SimpleBar className="twocolumn-iconview">
                                <Link to="#" className="logo">
                                    <img src={smallest} alt="" height="35" />
                                </Link>
                                {(navData || []).map((item: any, key: number) => (
                                    <React.Fragment key={key}>
                                        {item.icon && (
                                            <li>
                                                {item.subItems ? (
                                                    // If there are sub-items, handle expand/collapse and tooltip
                                                    <>
                                                        <Link
                                                            id={`tooltip-${item.id}`} // Unique ID for tooltip
                                                            onClick={(e) => {
                                                                e.preventDefault(); // Prevent default navigation if expanding
                                                                item.click(); // Handle collapse/expand
                                                            }}
                                                            to={item.link || "#"} // Provide navigation link here
                                                            sub-items={item.id}
                                                            className="nav-icon"
                                                        ...
                                                        >
                                                            {props.t(item.label)} {/* Tooltip content */}
                                                        </UncontrolledTooltip>
                                                    </>
                                                ) : (
                                                    // If there are no sub-items, just navigate and show tooltip
                                                    <>
                                                        <Link
                                                            id={`tooltip-${item.id}`} // Unique ID for tooltip
                                                            to={item.link || "/#"} // Direct navigation link
                                                            sub-items={item.id}
                                                            className="nav-icon"
                                                        >
                                                            <i className={item.icon}></i>
