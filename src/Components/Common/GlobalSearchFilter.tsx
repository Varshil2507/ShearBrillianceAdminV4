...
interface TaskStatusGlobalFilterProps {
    onFilterSubmit: (filters: { dateRange: Date[] | null; status: string }) => void;
    initialDateRange?: Date[]; // Optional prop for initial date range
    initialStatus?: string;    // Optional prop for initial status
}

const TaskStatusGlobalFilter: React.FC<TaskStatusGlobalFilterProps> = ({ onFilterSubmit,
    initialDateRange = [new Date()], // Default to today if no initial value provided
    initialStatus = "all" // Default to "pending" if no initial status is provided

}) => {
    const [dateRange, setDateRange] = useState<Date[] | null>(null); // State for date range
    const [status, setStatus] = useState<string>(initialStatus); // State for status
